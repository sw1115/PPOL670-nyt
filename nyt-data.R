...
nh_counties_long <- nh_counties_all %>% 
  pivot_longer(cols = c(3:36), names_to = "candidate", values_to = "votes")
head(nh_counties_long, n=20)

#pivot to wide data
nh_counties_wide <- nh_counties_long %>% 
  pivot_wider(id_cols = c(1:2), names_from = candidate, values_from = votes)
head(nh_counties_wide)
```
